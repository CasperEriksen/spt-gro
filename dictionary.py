...
class Dictionary:
	...
	def spell_suggestions(self, word, fromDanish, table, language):
		fromDanish = 1 if fromDanish else 2
		# Open data files
		db_path = self.dictionaries[language]['gddfile']
		db = sqlite3.connect(db_path)
		# SQL conditions
		sql_cond = " OR ".join(["word_ LIKE '%s'" % w for w in self.__edits1(word)])
		# SQL statement
		ss = "SELECT * FROM %s%i WHERE " % (table, fromDanish)
		ss += sql_cond
		# Create set of suggested words
		suggestions = set(w for _,w in list(db.execute(ss)))
		return suggestions
